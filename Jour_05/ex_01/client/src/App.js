...
class SeeAll extends React.Component {
  ...
  handleSearch = (e) => {
    e.preventDefault();
    var data = {
      blogde: this.props.match.params.handle,  
      recherche: this.refs.searchItem.value,
     }  
  ...
  render() {  
    if (this.state.new){
      return (
        <div className="container" id="contenutout">
        <h2>Bienvenue sur le blog de {this.state.user.login}</h2>
        
        <nav className="navbar navbar-light bg-light">
          <form onSubmit={this.handleSearch} className="form-inline"  style={{width: '95%'}}>
            <input style={{width: '90%'}}  className="form-control mr-sm-2" ref="searchItem" type="search" placeholder="Search" aria-label="Search"/>
