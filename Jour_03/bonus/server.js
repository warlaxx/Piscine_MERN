...
MongoClient.connect('mongodb://localhost:27017/mern-pool', { useUnifiedTopology: true }, function(err, client) {
    ...
    app.get('/boutique', function(req, res) {
        if (req.session.user != undefined) {
            sort = {};
            if (req.query.sort == "nm") {
                sort = { nom: 1 }
            } else if (req.query.sort == "pr") {
                sort = { prix: 1 }
            } else if (req.query.sort == "ct") {
                sort = { categorie: 1 }
            }
            db.collection("produits").find({}).sort(sort).toArray(function(err, results) {
    ...
    app.post('/boutique', function(req, res) {
        if (req.session.user != undefined) {
            ...
            db.collection("produits").find({ $or: [{ nom: req.body.search }, { description: req.body.search }, { categorie: req.body.search }] }).sort(sort).toArray(function(err, results) {
                res.render('boutique', { results: results });
            });
        } else {
            res.send("vous n'etes pas co")
